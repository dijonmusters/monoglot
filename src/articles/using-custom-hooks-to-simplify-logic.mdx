---
title: Using custom hooks to simplify logic
date: 2020-04-19
tags: [react, hooks]
---
...
This should look pretty familiar. We have a `data` variable that is being returned from our hook. Anytime we update the value of that variable - by using `setData` - it will trigger a re-render for anything consuming our hook. You can think of this as a `live` variable. Any component using that variable does not need to understand when or why it will change, but anytime it does change the component will be told to rerender with the new value. Magic!
...
Why don't we reactor our useRequest hook to provide this information, as determining the `loading` state of our `data` does feel like the responsibility of our request hook, rather than our rendering component. Plus it is super simple to do!
